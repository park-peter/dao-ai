...
class DatabricksProvider(ServiceProvider):
    ...
    def parse_documents(self, vector_store: VectorStoreModel) -> None:
        """
        Incrementally ingest documents from vector_store.source_path into the source table using docling UDFs.
        Follows the compound-ai-langgraph pattern: binary ingest, mime type detection, docling parsing, chunking, checkpointing.
        """
        ...
        vector_store.checkpoint_path.create()
        ...
        source_path: str = (
            vector_store.source_path.full_name
        )  # Use full_name for volume path
        checkpoint_path: str = vector_store.checkpoint_path.full_name

        # Create source table if it doesn't exist
        document_uri_col: str = vector_store.doc_uri or "source"

        logger.info(f"Creating source table {source_table} if it doesn't exist.")
        create_table_ddl: str = f"""
            CREATE TABLE IF NOT EXISTS {source_table} (
                id BIGINT GENERATED BY DEFAULT AS IDENTITY,
                {document_uri_col} STRING,
                mime_type STRING,
                content STRING
            ) TBLPROPERTIES (delta.enableChangeDataFeed = true)
        """
        logger.debug(f"Creating source table {source_table} with DDL: {create_table_ddl}")
        spark.sql(create_table_ddl)

        # Step 1: Read binary documents from source path using cloudFiles
        df: DataFrame = (
        ...
        df = df.selectExpr(
            f"path as {document_uri_col}", "mime_type", "chunks as content"
        )

        # Step 6: Write to source table with checkpointing (streaming)
        ...
        logger.info(f"Completed docling-style incremental ingest into {source_table}.")
